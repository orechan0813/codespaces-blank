...
export function JazzProvider({ children }: { children: ReactNode }) {
  ...
  const [practice, setPractice] = useState<PracticeItem[]>(seedPractice)
  const [announcements, setAnnouncements] = useState<Announcement[]>(seedAnnouncements)
  const [lostItems, setLostItems] = useState<LostItem[]>(seedLost)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (eventRows.length > 0) setEvents(eventRows)
      if (practiceRows.length > 0) setPractice(practiceRows)
      if (announcementRows.length > 0) setAnnouncements(announcementRows)
